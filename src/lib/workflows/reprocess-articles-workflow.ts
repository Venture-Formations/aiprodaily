...
async function finalizeIssue(issueId: string, moduleIds: string[], stepNum: number) {
  ...
  try {
    ...
    if (issueData?.publication_id) {
      ...
      console.log(`[Reprocess Step ${stepNum}] Text box modules initialized and generated`)
    }
  } catch (textBoxError) {
    console.log(`[Reprocess Step ${stepNum}] Text box generation skipped:`, textBoxError)
  }
