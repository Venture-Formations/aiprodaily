...
export const AI_PROMPTS = {
  ...
  // Secondary Article Evaluators (database-driven with fallbacks to primary prompts)
  secondaryContentEvaluator: async (post: { title: string; description: string; content?: string; hasImage?: boolean }) => {
    try {
      const { data, error } = await supabaseAdmin
        .from('app_settings')
        .select('value')
        .eq('key', 'ai_prompt_secondary_content_evaluator')
        .single()

      if (error || !data) {
        console.log('Using code fallback for secondaryContentEvaluator prompt')
        return FALLBACK_PROMPTS.contentEvaluator(post)
      }

      console.log('Using database prompt for secondaryContentEvaluator')
      const imagePenaltyText = post.hasImage
        ? 'This post HAS an image.'
        : 'This post has NO image - subtract 5 points from interest_level.'
      ...
      console.error('Error fetching secondaryContentEvaluator prompt, using fallback:', error)
      return FALLBACK_PROMPTS.contentEvaluator(post)
    }
  },

  secondaryCriteria1Evaluator: async (post: { title: string; description: string; content?: string }) => {
    try {
      const { data, error } = await supabaseAdmin
        .from('app_settings')
        .select('value')
        .eq('key', 'ai_prompt_secondary_criteria_1')
        .single()

      if (error || !data) {
        console.log('Using code fallback for secondaryCriteria1Evaluator prompt')
        return FALLBACK_PROMPTS.criteria1Evaluator(post)
      }

      console.log('Using database prompt for secondaryCriteria1Evaluator')
      return data.value
        .replace(/\{\{title\}\}/g, post.title)
        .replace(/\{\{description\}\}/g, post.description || 'No description available')
        .replace(/\{\{content\}\}/g, post.content ? post.content.substring(0, 1000) + '...' : 'No content available')
    } catch (error) {
      console.error('Error fetching secondaryCriteria1Evaluator prompt, using fallback:', error)
      return FALLBACK_PROMPTS.criteria1Evaluator(post)
    }
  },

  secondaryCriteria2Evaluator: async (post: { title: string; description: string; content?: string }) => {
    try {
      const { data, error } = await supabaseAdmin
        .from('app_settings')
        .select('value')
        .eq('key', 'ai_prompt_secondary_criteria_2')
        .single()

      if (error || !data) {
        console.log('Using code fallback for secondaryCriteria2Evaluator prompt')
        return FALLBACK_PROMPTS.criteria2Evaluator(post)
      }

      console.log('Using database prompt for secondaryCriteria2Evaluator')
      return data.value
        .replace(/\{\{title\}\}/g, post.title)
        .replace(/\{\{description\}\}/g, post.description || 'No description available')
        .replace(/\{\{content\}\}/g, post.content ? post.content.substring(0, 1000) + '...' : 'No content available')
    } catch (error) {
      console.error('Error fetching secondaryCriteria2Evaluator prompt, using fallback:', error)
      return FALLBACK_PROMPTS.criteria2Evaluator(post)
    }
  },

  secondaryCriteria3Evaluator: async (post: { title: string; description: string; content?: string }) => {
    try {
      const { data, error } = await supabaseAdmin
        .from('app_settings')
        .select('value')
        .eq('key', 'ai_prompt_secondary_criteria_3')
        .single()

      if (error || !data) {
        console.log('Using code fallback for secondaryCriteria3Evaluator prompt')
        return FALLBACK_PROMPTS.criteria3Evaluator(post)
      }

      console.log('Using database prompt for secondaryCriteria3Evaluator')
      return data.value
        .replace(/\{\{title\}\}/g, post.title)
        .replace(/\{\{description\}\}/g, post.description || 'No description available')
        .replace(/\{\{content\}\}/g, post.content ? post.content.substring(0, 1000) + '...' : 'No content available')
    } catch (error) {
      console.error('Error fetching secondaryCriteria3Evaluator prompt, using fallback:', error)
      return FALLBACK_PROMPTS.criteria3Evaluator(post)
    }
  },

  secondaryCriteria4Evaluator: async (post: { title: string; description: string; content?: string }) => {
    try {
      const { data, error } = await supabaseAdmin
        .from('app_settings')
        .select('value')
        .eq('key', 'ai_prompt_secondary_criteria_4')
        .single()

      if (error || !data) {
        console.log('Using code fallback for secondaryCriteria4Evaluator prompt')
        return FALLBACK_PROMPTS.criteria4Evaluator(post)
      }

      console.log('Using database prompt for secondaryCriteria4Evaluator')
      return data.value
        .replace(/\{\{title\}\}/g, post.title)
        .replace(/\{\{description\}\}/g, post.description || 'No description available')
        .replace(/\{\{content\}\}/g, post.content ? post.content.substring(0, 1000) + '...' : 'No content available')
    } catch (error) {
      console.error('Error fetching secondaryCriteria4Evaluator prompt, using fallback:', error)
      return FALLBACK_PROMPTS.criteria4Evaluator(post)
    }
  },

  secondaryCriteria5Evaluator: async (post: { title: string; description: string; content?: string }) => {
    try {
      const { data, error } = await supabaseAdmin
        .from('app_settings')
        .select('value')
        .eq('key', 'ai_prompt_secondary_criteria_5')
        .single()

      if (error || !data) {
        console.log('Using code fallback for secondaryCriteria5Evaluator prompt')
        return FALLBACK_PROMPTS.criteria5Evaluator(post)
      }

      console.log('Using database prompt for secondaryCriteria5Evaluator')
      return data.value
        .replace(/\{\{title\}\}/g, post.title)
        .replace(/\{\{description\}\}/g, post.description || 'No description available')
        .replace(/\{\{content\}\}/g, post.content ? post.content.substring(0, 1000) + '...' : 'No content available')
    } catch (error) {
      console.error('Error fetching secondaryCriteria5Evaluator prompt, using fallback:', error)
      return FALLBACK_PROMPTS.criteria5Evaluator(post)
    }
  },

  secondaryNewsletterWriter: async (post: { title: string; description: string; content?: string; source_url?: string }) => {
    try {
      const { data, error } = await supabaseAdmin
        .from('app_settings')
        .select('value')
        .eq('key', 'ai_prompt_secondary_newsletter_writer')
        .single()

      if (error || !data) {
        console.log('Using code fallback for secondaryNewsletterWriter prompt')
        return FALLBACK_PROMPTS.newsletterWriter(post)
      }

      console.log('Using database prompt for secondaryNewsletterWriter')
      return data.value
        .replace(/\{\{title\}\}/g, post.title)
        .replace(/\{\{description\}\}/g, post.description || 'No description available')
        .replace(/\{\{content\}\}/g, post.content ? post.content.substring(0, 1500) + '...' : 'No additional content')
      ...
      console.error('Error fetching secondaryNewsletterWriter prompt, using fallback:', error)
      return FALLBACK_PROMPTS.newsletterWriter(post)
    }
  }
}
