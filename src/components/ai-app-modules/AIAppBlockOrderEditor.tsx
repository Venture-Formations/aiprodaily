...
import type { AIAppBlockType } from '@/types/database'

interface AIAppBlockOrderEditorProps {
  blockOrder: AIAppBlockType[]
  onChange: (newOrder: AIAppBlockType[]) => void
  disabled?: boolean
}
...
const ALL_BLOCK_TYPES: AIAppBlockType[] = ['title', 'logo', 'image', 'tagline', 'description', 'button']
...
export default function AIAppBlockOrderEditor({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      const oldIndex = blockOrder.indexOf(active.id as AIAppBlockType)
      const newIndex = blockOrder.indexOf(over?.id as AIAppBlockType)
  ...
  const handleRemoveBlock = (block: AIAppBlockType) => {
