...
import SelectionModeEditor from './SelectionModeEditor'
import CooldownEditor from './CooldownEditor'
import type { AdModule, AdBlockType, AdSelectionMode } from '@/types/database'
import { useAdModuleSettings } from './useAdModuleSettings'

interface AdModuleSettingsProps {
  module: AdModule
  publicationId: string
  onUpdate: (updates: Partial<AdModule>) => Promise<void>
  onDelete: () => void
  cooldownDays: number
  onCooldownChange: (days: number) => Promise<void>
}

export default function AdModuleSettings({
  module,
  publicationId,
  onUpdate,
  onDelete,
  ...
  const {
    ...
    handleDelete
  } = useAdModuleSettings(module, onUpdate, onDelete)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-4 border-b">
        <div className="flex-1 flex items-center gap-2">
          <input
            ...
            disabled={saving}
            className="text-xl font-semibold text-gray-900 bg-transparent border-none focus:outline-none focus:ring-2 focus:ring-blue-500 rounded px-1 -ml-1"
          />
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              localModule.is_active ? 'bg-blue-600' : 'bg-gray-200'
      ...
      <div className="space-y-4">
        <BlockOrderEditor
          blockOrder={localModule.block_order as AdBlockType[]}
          onChange={handleBlockOrderChange}
          disabled={saving}
        />

        <SelectionModeEditor
          value={localModule.selection_mode as AdSelectionMode}
      ...
      <div className="border border-red-200 rounded-lg overflow-hidden mt-8">
        <div className="p-4 bg-red-50">
          ...
          <p className="text-sm text-red-600 mt-1">
            Deleting this section will orphan {adCount ?? '...'} ads. They can be reassigned later.
          </p>
        </div>
