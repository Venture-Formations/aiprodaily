...
import { useSparkLoopRecModuleSettings } from './useSparkLoopRecModuleSettings'
...
export default function SparkLoopRecModuleSettings({ module, onUpdate, onDelete }: SparkLoopRecModuleSettingsProps) {
  const {
  ...
  } = useSparkLoopRecModuleSettings({ module, onUpdate, onDelete })
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-4 border-b">
        <div className="flex-1 flex items-center gap-2">
          <input
            type="text" value={localModule.name}
            onChange={(e) => setLocalName(e.target.value)}
            onBlur={(e) => handleNameChange(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter') { handleNameChange(localModule.name); (e.target as HTMLInputElement).blur() } }}
            disabled={saving}
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleActiveToggle} disabled={saving} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${localModule.is_active ? 'bg-orange-600' : 'bg-gray-200'} ${saving ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
      ...
      <div>
        ...
        <div className="space-y-2">
          {SELECTION_MODES.map(mode => (
            <label key={mode.value} className={`flex items-start gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors ${localModule.selection_mode === mode.value ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300'}`}>
              <input type="radio" name="selection_mode" value={mode.value} checked={localModule.selection_mode === mode.value} onChange={() => handleSelectionModeChange(mode.value)} disabled={saving} className="mt-0.5 text-orange-600 focus:ring-orange-500" />
              <div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Recommendations Per Issue</label>
        <div className="flex items-center gap-3">
          <input type="number" min={1} max={10} value={localModule.recs_count} onChange={(e) => setLocalRecsCount(parseInt(e.target.value) || 3)} onBlur={(e) => handleRecsCountChange(parseInt(e.target.value) || 3)} disabled={saving} className="w-20 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-center" />
      ...
      <div className="border border-red-200 rounded-lg overflow-hidden mt-8">
        <div className="p-4 bg-red-50">
          ...
          <p className="text-sm text-red-600 mt-1">Deleting this SparkLoop recommendations section will remove it from all future issues.</p>
        </div>
        {!deleteConfirm ? (
