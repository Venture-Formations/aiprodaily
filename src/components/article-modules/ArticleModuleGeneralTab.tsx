...
const SELECTION_MODE_OPTIONS: { value: ArticleSelectionMode; label: string; description: string }[] = [
  { value: 'top_score', label: 'Top Score', description: 'Automatically selects highest-scoring articles based on criteria ratings' },
  { value: 'manual', label: 'Manual', description: 'You manually select which articles to include for each issue' }
]

const BLOCK_LABELS: Record<ArticleBlockType, string> = {
  source_image: 'Source Image', ai_image: 'AI Image', trade_image: 'Trade Image', title: 'Title', body: 'Body'
}

export default function ArticleModuleGeneralTab({ module, onUpdate, disabled = false }: ArticleModuleGeneralTabProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <button onClick={() => setBlockOrderOpen(!blockOrderOpen)} className="w-full flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 transition-colors">
          ...
          <svg className={`w-5 h-5 text-gray-500 transition-transform ${blockOrderOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          </svg>
        </button>
        {blockOrderOpen && (
          <div className="p-4 border-t border-gray-100">
            <ArticleBlockOrderEditor blockOrder={module.block_order as ArticleBlockType[]} onChange={(newOrder) => withSaving(() => onUpdate({ block_order: newOrder }))} disabled={isDisabled} />
          </div>
        )}
      </div>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <button onClick={() => setSelectionModeOpen(!selectionModeOpen)} className="w-full flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 transition-colors">
          <div className="flex items-center gap-2">
            <span className="font-medium text-gray-700">Selection Mode</span>
            {currentMode && <span className="text-xs px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full">{currentMode.label}</span>}
          </div>
          <svg className={`w-5 h-5 text-gray-500 transition-transform ${selectionModeOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          </svg>
        </button>
        {selectionModeOpen && (
          <div className="p-4 space-y-2">
            {SELECTION_MODE_OPTIONS.map(option => (
              <label key={option.value} className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-colors ${module.selection_mode === option.value ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'} ${isDisabled ? 'opacity-60 cursor-not-allowed' : ''}`}>
                <input type="radio" name="articleSelectionMode" value={option.value} checked={module.selection_mode === option.value} onChange={(e) => withSaving(() => onUpdate({ selection_mode: e.target.value as ArticleSelectionMode }))} disabled={isDisabled} className="mt-1" />
                <div><span className="font-medium text-gray-700">{option.label}</span><p className="text-sm text-gray-500 mt-0.5">{option.description}</p></div>
              </label>
            ))}
          </div>
        )}
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <div className="p-4 bg-gray-50"><span className="font-medium text-gray-700">Article Selection Settings</span></div>
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between">
            <div><label className="text-sm font-medium text-gray-700">Articles per Issue</label><p className="text-xs text-gray-500">Number of articles to include in this section</p></div>
            <input type="number" min={1} max={10} value={module.articles_count} onChange={(e) => { const val = parseInt(e.target.value); if (val >= 1 && val <= 10) withSaving(() => onUpdate({ articles_count: val })) }} disabled={isDisabled} className="w-20 px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
          </div>
          <div className="flex items-center justify-between">
            <div><label className="text-sm font-medium text-gray-700">RSS Lookback (Hours)</label><p className="text-xs text-gray-500">How far back to look for posts</p></div>
            <input type="number" min={24} max={168} step={12} value={module.lookback_hours} onChange={(e) => { const val = parseInt(e.target.value); if (val >= 24 && val <= 168) withSaving(() => onUpdate({ lookback_hours: val })) }} disabled={isDisabled} className="w-20 px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
          </div>
          {(() => {
            ...
            const handleMultiplierChange = (value: number | null) => withSaving(async () => {
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{isCustom ? `Fetches ${articlesCount} + ${candidateMultiplier} = ${articlesCount + candidateMultiplier} candidate posts` : `Default: fetches ${articlesCount} \u00d7 4 = ${articlesCount * 4} candidate posts`}</p>
                  </div>
                  <button type="button" onClick={() => handleMultiplierChange(isCustom ? null : articlesCount * 3)} disabled={isDisabled} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${isCustom ? 'bg-emerald-500' : 'bg-gray-300'} ${isDisabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}`}>
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${isCustom ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
                {isCustom && (
                  <div className="flex items-center gap-2 pl-1">
                    <span className="text-xs text-gray-500">Extra posts:</span>
                    <input type="number" min={0} max={50} value={candidateMultiplier} onChange={(e) => { const val = parseInt(e.target.value); if (!isNaN(val) && val >= 0 && val <= 50) handleMultiplierChange(val) }} disabled={isDisabled} className="w-20 px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                  </div>
                )}
              </div>
            )
...
function RssOutputSection({ module, onUpdate, disabled, saving, setSaving, variant }: { module: ArticleModule; onUpdate: (updates: Partial<ArticleModule>) => Promise<void>; disabled: boolean; saving: boolean; setSaving: (v: boolean) => void; variant: RssVariant }) {
  ...
  const updateRssConfig = useCallback(async (updates: Record<string, any>) => {
    ...
    try {
      ...
      await onUpdate({ config: { ...currentConfig, rss_output: { ...currentRss, ...updates } } } as any)
          ...
          {enabled && (
            <>
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <input type="text" readOnly value={feedUrl} className="flex-1 px-3 py-1.5 border border-gray-300 rounded-lg text-xs font-mono bg-gray-50 text-gray-600" />
