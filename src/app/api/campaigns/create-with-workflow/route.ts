...
// Step functions (copied from process-rss-workflow.ts)
async function generatePrimaryTitles(campaignId: string) {
  "use step"
  console.log('[Workflow] Generating 6 primary titles...')
...
async function generatePrimaryBodiesBatch1(campaignId: string) {
  "use step"
  console.log('[Workflow] Generating 3 primary bodies (batch 1)...')
  const { RSSProcessor } = await import('@/lib/rss-processor')
  const processor = new RSSProcessor()
...
async function generatePrimaryBodiesBatch2(campaignId: string) {
  "use step"
  console.log('[Workflow] Generating 3 primary bodies (batch 2)...')
  const { RSSProcessor } = await import('@/lib/rss-processor')
  const processor = new RSSProcessor()
...
async function factCheckPrimary(campaignId: string) {
  "use step"
  console.log('[Workflow] Fact-checking primary articles...')
  const { RSSProcessor } = await import('@/lib/rss-processor')
  const processor = new RSSProcessor()
...
async function generateSecondaryTitles(campaignId: string) {
  "use step"
  console.log('[Workflow] Generating 6 secondary titles...')
  const { RSSProcessor } = await import('@/lib/rss-processor')
  const processor = new RSSProcessor()
...
async function generateSecondaryBodiesBatch1(campaignId: string) {
  "use step"
  console.log('[Workflow] Generating 3 secondary bodies (batch 1)...')
  const { RSSProcessor } = await import('@/lib/rss-processor')
  const processor = new RSSProcessor()
...
async function generateSecondaryBodiesBatch2(campaignId: string) {
  "use step"
  console.log('[Workflow] Generating 3 secondary bodies (batch 2)...')
  const { RSSProcessor } = await import('@/lib/rss-processor')
  const processor = new RSSProcessor()
...
async function factCheckSecondary(campaignId: string) {
  "use step"
  console.log('[Workflow] Fact-checking secondary articles...')
  const { RSSProcessor } = await import('@/lib/rss-processor')
  const processor = new RSSProcessor()
...
async function finalizeCampaign(campaignId: string) {
  "use step"
  console.log('[Workflow] Finalizing campaign...')
  const { RSSProcessor } = await import('@/lib/rss-processor')
  ...
  await processor.unassignUnusedPosts(campaignId)

  console.log('[Workflow] ✓ Finalized')
}
