...
async function initializeTextBoxModules(issueId: string, publicationId: string) {
  try {
    ...
    // Generate AI content for 'before_articles' timing blocks
    // (after_articles blocks need article context and are generated by the workflow)
    const result = await TextBoxGenerator.generateBlocksWithTiming(issueId, 'before_articles')
    console.log(`Text box modules initialized, generated ${result.generated} before_articles blocks`)
  } catch (error) {
