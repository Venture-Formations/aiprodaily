...
/**
 * GET /api/cron/post-to-facebook
 * Daily cron job to post ad content to Facebook Pages
 * Runs every 5 minutes and checks if it's time to post for each publication
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[Facebook] Request type:', isVercelCron ? 'Vercel Cron' : 'Manual Test')
    ...
    return NextResponse.json(
      {
        error: 'Facebook posting cron failed',
...
// Also support POST for manual triggering with auth header
