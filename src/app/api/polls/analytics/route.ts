...
export const GET = withApiHandler(
  { authTier: 'authenticated', logContext: 'polls/analytics' },
  async ({ request }) => {
    ...
    if (newsletterError || !newsletter) {
      console.error('[Poll Analytics] Newsletter not found:', newsletterSlug)
      return NextResponse.json(
    ...
    const pollAnalytics = polls.map(poll => {
      ...
      if (pollIssues.length > 0) {
        ...
        if (totalRecipients > 0) {
          // Use 1 decimal place for small percentages (poll response rates are typically <1%)
          responseRate = Math.round((uniqueRespondents / totalRecipients) * 1000) / 10
        }
      }
