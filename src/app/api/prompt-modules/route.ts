  ...
  try {
    ...
    const { data: module, error } = await supabaseAdmin
      ...
      .insert({
        ...
        selection_mode: body.selection_mode ?? 'random',
        block_order: body.block_order ?? ['title', 'body'],
        config: body.config ?? {},
        next_position: body.next_position ?? 1
      })
      .select()
      .single()
    ...
    console.log(`[PromptModules] Created module: ${module.name} (${module.id})`)
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    for (const item of body.modules) {
      ...
      if (error) {
        console.error(`[PromptModules] Failed to update order for ${item.id}:`, error)
      }
    }
