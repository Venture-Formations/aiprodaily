      ...
      if (providerSettings.provider === 'beehiiv') {
        ...
        if (beehiivPublicationId && beehiivApiKey) {
          ...
          if (!result.success && result.error === 'Subscriber not found') {
            console.log(`[SparkLoop Recommend Subscribe] Subscriber ${email} not found in Beehiiv, waiting 2s and retrying...`)
            await new Promise(resolve => setTimeout(resolve, 2000))
            result = await updateBeehiivSubscriberField(email, 'sparkloop', 'true', beehiivPublicationId, beehiivApiKey)
          }
          if (result.success) {
            console.log(`[SparkLoop Recommend Subscribe] Updated Beehiiv SparkLoop field for ${email}`)
          } else {
            console.error('[SparkLoop Recommend Subscribe] Failed to update Beehiiv field:', result.error)
          }
        }
      } else {
        ...
        if (!result.success && result.error === 'Subscriber not found') {
          console.log(`[SparkLoop Recommend Subscribe] Subscriber ${email} not found, waiting 2s and retrying...`)
          await new Promise(resolve => setTimeout(resolve, 2000))
          result = await mailerlite.updateSubscriberField(email, 'sparkloop', 'true')
        }
        if (result.success) {
          console.log(`[SparkLoop Recommend Subscribe] Updated MailerLite SparkLoop field for ${email}`)
        } else {
          console.error('[SparkLoop Recommend Subscribe] Failed to update MailerLite field:', result.error)
        }
      }
    } catch (providerError) {
