...
export default function NewslettersPage() {
  ...
  const fetchHeaderImage = async () => {
    try {
      const response = await fetch('/api/settings/header-image')
      if (response.ok) {
        const data = await response.json()
      ...
      <section id="newsletters" className="pt-20 py-10 px-4 sm:px-6 lg:px-8 bg-[#F5F5F7]">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center space-y-2 mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#1D1D1F] text-balance">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-20">
              <div className="text-[#1D1D1F]/60">Loading newsletters...</div>
            </div>
