...
// --- Image Upload Component (with existingImageUrl support) ---
...
export function ClaimModal({ isOpen, onClose, toolId, initialData }: ClaimModalProps) {
  const { user } = useUser()
      ...
      <div className="relative bg-white rounded-2xl shadow-xl max-w-2xl w-full mx-4 my-8 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <input type="text" id="toolName" value={formData.toolName}
              onChange={(e) => setFormData(prev => ({ ...prev, toolName: e.target.value }))}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              placeholder="e.g., My AI Tool" disabled={isSubmitting} />
          </div>
          ...
          <div>
            ...
            <input type="text" id="websiteUrl" value={formData.websiteUrl}
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              placeholder="yourwebsite.com" disabled={isSubmitting} />
            <p className="mt-1 text-sm text-gray-500">https:// will be added automatically if not included</p>
          </div>

          <div>
            <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Description * (max 200 characters)</label>
            <textarea id="description" value={formData.description}
              onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))}
              rows={4} maxLength={200}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              placeholder="Describe what your tool does and how it helps accountants..." disabled={isSubmitting} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <ImageUpload
              label="Logo Image" description="Square logo for list cards (1:1 ratio)"
              aspectRatio={1} aspectLabel="1:1 square"
              ...
              onClear={() => { setLogoBlob(null); setLogoPreview(null); setKeepExistingLogo(false) }}
              disabled={isSubmitting}
            />
            <ImageUpload
              label="Listing Image" description="Wide image for tool detail page (16:9 ratio)"
              aspectRatio={16 / 9} aspectLabel="16:9 wide"
              ...
              onClear={() => { setListingBlob(null); setListingPreview(null); setKeepExistingImage(false) }}
              disabled={isSubmitting}
            />
          </div>
