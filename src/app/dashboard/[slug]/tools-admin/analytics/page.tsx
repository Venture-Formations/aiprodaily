...
import { useToolsAnalytics } from './useToolsAnalytics'
      ...
      <div className="px-4 py-6 sm:px-0">
        ...
        <div className="mb-6">
          <Link href="../tools-admin" className="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 mb-4">
            <ArrowLeft className="w-4 h-4 mr-1" /> Back to Tools Admin
          </Link>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={days} onChange={(e) => setDays(parseInt(e.target.value))} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-red-50 rounded-lg">
            ...
            <button onClick={fetchAnalytics} className="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Retry</button>
          </div>
        ) : analytics ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-sm text-gray-500">Total Clicks</p><p className="text-2xl font-bold text-gray-900">{formatNumber(analytics.totalClicks)}</p></div>
                </div>
              </div>
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-green-100 rounded-lg"><Eye className="w-5 h-5 text-green-600" /></div>
                  <div><p className="text-sm text-gray-500">Tool Views</p><p className="text-2xl font-bold text-gray-900">{formatNumber(analytics.clicksByType.tool_view)}</p></div>
                </div>
              </div>
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-purple-100 rounded-lg"><ExternalLink className="w-5 h-5 text-purple-600" /></div>
                  <div><p className="text-sm text-gray-500">Website Clicks</p><p className="text-2xl font-bold text-gray-900">{formatNumber(analytics.clicksByType.external_link)}</p></div>
                </div>
              </div>
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-orange-100 rounded-lg"><MousePointer className="w-5 h-5 text-orange-600" /></div>
                  <div><p className="text-sm text-gray-500">Conversion Rate</p><p className="text-2xl font-bold text-gray-900">{getConversionRate()}</p></div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="h-48 flex items-end gap-1">
                {getDailyData().map(({ date, clicks }) => (
                  <div key={date} className="flex-1 bg-blue-500 rounded-t hover:bg-blue-600 transition-colors relative group" style={{ height: `${(clicks / getMaxClicks()) * 100}%`, minHeight: clicks > 0 ? '4px' : '0' }}>
                    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-10">{date}: {clicks} clicks</div>
                  </div>
                ))}
              </div>
              <p className="text-xs text-gray-500 mt-2 text-center">{analytics.dateRange.start} to {analytics.dateRange.end}</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {analytics.topToolsByViews.map((tool, index) => (
                      <div key={tool.id} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <Link href={`/tools/${tool.id}`} target="_blank" className="text-sm font-medium text-gray-900 hover:text-blue-600">{tool.name}</Link>
                        </div>
                  ...
                  <div className="space-y-3">
                    {analytics.topCategories.map((category, index) => (
                      <div key={category.slug} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-medium text-gray-400 w-6">{index + 1}.</span>
                          <Link href={`/tools/category/${category.slug}`} target="_blank" className="text-sm font-medium text-gray-900 hover:text-blue-600">{category.name}</Link>
                        </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                ...
                {Object.keys(analytics.clicksByReferrerType).length > 0 ? (
                  <div className="space-y-3">
                    {Object.entries(analytics.clicksByReferrerType).sort((a, b) => b[1] - a[1]).map(([referrer, clicks]) => (
                      <div key={referrer} className="flex items-center justify-between">
                        <span className="text-sm text-gray-900 capitalize">{referrer.replace(/_/g, ' ')}</span>
