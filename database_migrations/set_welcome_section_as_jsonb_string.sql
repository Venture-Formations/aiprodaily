-- Store welcome section as JSONB string (like all other prompts)
-- This makes it consistent with factChecker, criteria prompts, etc.

-- Step 1: Build the JSONB object
-- Step 2: Convert it to text (JSON string representation)
-- Step 3: Wrap that text in a JSONB string value using to_jsonb()
...
Task: Write a conversational, friendly introduction that includes 3–5 of the most compelling stories. Follow the format and tone described above. Return your response as valid JSON with only the keys intro, tagline, and summary.'
  ...
  )::text  -- Convert JSONB object to text (JSON string)
)  -- Wrap the JSON string in a JSONB string value
WHERE key = 'ai_prompt_welcome_section';

-- Verify it's now a JSONB string (like other prompts)
SELECT
  key,
  jsonb_typeof(value) as value_type,
  LEFT(value::text, 50) as preview
FROM app_settings
WHERE key = 'ai_prompt_welcome_section';
