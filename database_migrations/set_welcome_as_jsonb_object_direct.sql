-- Set welcome section as a JSONB object (not a string)
-- This replaces the entire value with the correct structure
...
SET value = jsonb_build_object(
  ...
  'messages', jsonb_build_array(
    jsonb_build_object(
      'role', 'system',
      'content', 'You are writing a welcoming introduction for AI Accounting Daily — a daily newsletter about how AI is transforming the accounting world.
...
- Ensure the summary reads naturally and ends smoothly.'
  ...
  "summary": "Today, we''re covering AI assistants in audit, the rise of predictive analytics, and how client onboarding is changing fast."
    ...
    jsonb_build_object(
      'role', 'user',
      'content', 'ARTICLES TO SUMMARIZE (Primary and Secondary): {{articles}}
